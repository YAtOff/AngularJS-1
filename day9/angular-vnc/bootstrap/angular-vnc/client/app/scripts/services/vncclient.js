(function () {
  ...
  function VNCClient($q, Io) {
    ...
    this.connect = function connect(config) {
    ...
    VNCClient.prototype.disconnect = function disconnect() {
      this.socket.disconnect();
      this.connected = false;
    };
    this.addFrameCallback = function addFrameCallback(cb) {
      this.frameCallbacks.push(cb);
    };
    this.removeFrameCallback = function removeFrameCallback(cb) {
      this.frameCallbacks = this.frameCallbacks.filter(function (c) {
        return c !== cb;
      });
    };
    this.update = function update(frame) {
      this.frameCallbacks.forEach(function (cb) {
        cb.call(null, frame);
      });
    };
    this.sendMouseEvent = function sendMouseEvent(x, y, mask) {
      this.socket.emit('mouse', {x: x, y: y, button: mask});
    };
    this.sendKeyboardEvent = function sendKeyboardEvent(code, shift, isDown) {
      var rfbKey = this.toRfbKeyCode(code, shift);
      if (rfbKey) {
